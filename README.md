...
The recorder samples raw telemetry data at a given frequency and stores it on
disk in a compressed format. The player replays previously recorded sessions by
...
rely on shared memory as well as reading files saved by recorder directly to
enable unit testing.
...
Record data with `ksana record` and replay it with `ksana play` in a
reproducible way as many times as you need for debugging. Here's how it looks
with REC recorder:
...
Commands:
  record  Record shared memory to file (default)
  play  Play back recorded file to shared memory
  inspect  Inspect recorded file and print info about it
...
console window that runs recorder and press `Ctrl+C`. The message
...
`Recorder stopped. You can now close this window`. Don't close the console
window with the recorder running or the output file will be corrupted. The
output file will be saved in the same folder with the recorder binary with the
name started with `ksana_`, current date and time in the name and `.ksr` ("ksana
recording") extension.

Available options:

```
> .\ksana.exe record --help
...
Reads the specified file (generated by recorder) and outputs data to shared
memory as if it was generated by the simulator allowing third-party apps to
...
Reads the specified file (generated by recorder) and prints the basic
information about it in the standard output.

```
>.\ksana.exe inspect --help
Inspect recorded file and print info about it

Usage: ksana.exe inspect --input <INPUT>

Options:
  -i, --input <INPUT>  Input file to inspect
  -h, --help           Print help
```
...
```
>.\ksana.exe .\ksana.exe inspect --input ksana_irac_20260319_09_16_39.bin
Ksana recording: ksana_irac_20260319_09_16_39.bin (sim: irac, fps: 5)
Total frames: 10246
...
Copyright (c) Dmitriy Linev 2025-2026
